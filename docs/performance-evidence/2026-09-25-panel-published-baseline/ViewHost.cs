            ...
            switch (node.Control)
            {
                ...
                case Panel panel:
                    ...
                    try
                    {
                        ...
                        Dictionary<UIElement, int>? targetPositions = null;
                        for (var i = 0; i < next.Count; i++)
                        {
                            var element = next[i].Element;
                            if (i >= panel.Children.Count || !ReferenceEquals(panel.Children[i], element))
                            {
                                // A forward move (e.g. first row to last) leaves the next
                                // desired child adjacent. Move the displaced row once instead
                                // of detaching every intervening themed subtree.
                                if (i + 1 < panel.Children.Count && ReferenceEquals(panel.Children[i + 1], element))
                                {
                                    if (targetPositions is null)
                                    {
                                        targetPositions = new(next.Count);
                                        for (var position = 0; position < next.Count; position++)
                                            targetPositions.Add(next[position].Element, position);
                                    }
                                    var displaced = panel.Children[i];
                                    if (targetPositions.TryGetValue(displaced, out var destination) && destination > i)
                                    {
                                        panel.Children.RemoveAt(i);
                                        panel.Children.Insert(Math.Min(destination, panel.Children.Count), displaced);
                                    }
                                }
                                if (i >= panel.Children.Count || !ReferenceEquals(panel.Children[i], element))
                                {
