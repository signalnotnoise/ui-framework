...
$oldVersion = $referenceVersion.Trim([char[]]'[]')
if ($CandidateVersion -notmatch '^\d+\.\d+\.\d+(-[0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?$') { throw 'Invalid candidate version.' }
if ($oldVersion -eq $CandidateVersion) { throw 'Candidate must differ from the installed package.' }
...
if ($UseWorkingTree) {
    $files = (& git -C $consumer -c core.quotepath=false ls-files --cached --others --exclude-standard) | ForEach-Object { $_.Trim('"') }
    if ($LASTEXITCODE -ne 0) { throw 'Cannot enumerate consumer sources.' }
    foreach ($file in $files) {
        $source = Join-Path $consumer $file
        if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw 'Commit deletions before a working-tree comparison.' }
...
foreach ($side in @('baseline','candidate')) {
    ...
    if ($side -eq 'candidate') {
        ...
        [IO.File]::WriteAllText($project, $text.Replace($oldReference,
            'Include="SignalNotNoise.UI.Wpf" Version="' + $CandidateVersion + '"'))
        foreach ($package in $packages) {
            $destination = Join-Path $app "vendor/ui-framework/$($package.Name)"
            if (Test-Path $destination) { throw 'Never overwrite an archived package.' }
...
foreach ($scenario in $scenarios.Keys) {
    for ($sample=0; $sample -le $Samples; $sample++) {
        ...
        foreach ($side in $order) {
            ...
            if ($result.ExperimentalComCleanup -ne $false -or $result.Students -ne 1000 -or $result.Files -ne 1000 -or
